...
//
// Registers 13 native model tools wrapping the `codegraph` CLI
...
// falls back to the `shell` service (bash). Both are read via ctx.get with an
// undefined check — no hard injection, so the plugin still mounts if a
// deployment lacks one of the executors. The `codegraph` executable must be on
// PATH (e.g. global `npm i -g @colbymchenry/codegraph` or the npm thin shim);
// a clearer error is thrown when it is missing.
...
// Plugin config. guideSearch defaults to true: when set, a high-priority
// system-prompt section tells the model to reach for codegraph_* before
// grep/glob/read for code search. Set false (e.g. in cordis.patch.yml) to
// register the tools without that guidance.
...
const CODEGRAPH_GUIDE = `Prefer codegraph_* tools over grep/glob/read for searching, exploring, and understanding code:
- First, run codegraph_status for the current project. If it reports initialized:false, run codegraph_init to build the on-disk code index once (requires the codegraph CLI on PATH).
- Then search/understand with the indexed graph instead of brute-force grepping: codegraph_query to locate symbols, codegraph_explore to map an area of the codebase by intent, codegraph_node to pull one symbol/file with its caller·callee trail, codegraph_callers/codegraph_callees to answer "who uses / who calls", codegraph_impact before refactoring, and codegraph_affected to find tests after editing source.
- After editing source files, run codegraph_sync so later codegraph_* queries reflect the new code.
These return structured, dependency-aware results far faster and more completely than grep + read. Only fall back to grep/glob/read when the graph is unavailable for the path in question.`
